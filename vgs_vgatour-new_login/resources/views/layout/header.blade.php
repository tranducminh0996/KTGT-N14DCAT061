                                        ...
                                        <div class="row">
                                            <div class="col-md-4">
                                                <ul class="nav flex-column">
                                                    @foreach($itemSystem->listTour as $key1=> $tour)
                                                    <li class="li-tour" ><a class="{{($key1==0)?'active':''}}" style="text-decoration: none; color: #fff" data-toggle="tab" href="{{'#tour'.$tour->id}}">{{$tour->name}}</a></li>
                                                    <br>
                                            ...
                                            <div class="tab-content col-md-4">
                                                @foreach($itemSystem->listTour as $key2=> $tour)
                                                <?php $cover = getTourBanner($tour->id, '8') ; ?>
                                                <div id="{{'tour'.$tour->id}}" class="tab-pane fade {{($key2==0)?'active show':''}}">
                                                    <div class="img-tour-menu">
                                                        <img src="<?php if(isset($cover['link_image'])) echo $cover['link_image']  ?>" alt="">
                                                    </div>
                                                    <div class="content-tour-menu">
                                                        <p class="text-center">{{$tour->name}}</p>
                                                        <div class="info-tour-select">
                                                            <p>
                                                                {{__('santhidau')}}
                                                                : <?=getFacility($tour->facility_id)?>
                                                            </p>
                                                            <p>
                                                                {{__('ngaythidau')}}: <?=getTourDate($tour->id)?>
                                                            </p>
                                                            <p>
                                                                {{__('soluongvandongvien')}}
                                                                : {{$tour->number_athletic}}
                                                            </p>
                                                            <p>{{$tour->describe}}</p>
                                                        </div>

                                                        <a href="{{route('home', ['tour' => $tour->id])}}" class="btn btn-go-to-event">
