...
@section('content')
    <div class="post-wrap">
        <div class="breadcrumb-wrap">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"> {{__('suabaiviet')}} #{{$video->id}}</li>
                </ol>
            </nav>
        </div>
        <div class="error">
        ...
        <div class="app">
            <form method="POST" action="{{route('video.edit', $video->id)}}" accept-charset="UTF-8">
                @csrf
                    ...
                    <div class="row">
                                ...
                                <div class="panels">
                                    <div class="panel" id="one-panel">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control input-name" placeholder="{{$video->name}}"
                                                   name="name"
                                        ...
                                        <div class="form-group ">
                                            <div id="edit-slug-box">
                                                <label class="control-label required" for="current-slug"
                                                       aria-required="true">{{__('duongdan')}}:</label>
                                                <span id="sample-permalink">
                                                    <a class="permalink" target="_blank"
                                                       href="">
                                                        <span class="default-slug">
                                                            https://vgstour.com/<span
                                                                id="editable-post-name">{{$video->slug}}</span>
                                                        </span>
                                                    </a>
                                                </span>
                                                <input type="hidden" id="current-slug" name="slug" class=""
                                                       value="{{$video->slug}}">
                                                <span id="edit-slug-buttons">
                                                    <button type="button" class="btn btn-secondary"
                                                            id="change_slug">Sửa</button>
                                                    <button type="button" class="save btn btn-secondary"
                                                            id="btn-ok">OK</button>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" data-counter="120" name="video_url"
                                                   type="text" value="{{$video->video_url}}" id="video_url"
                                                   aria-invalid="false">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input class="form-control featured" placeholder="{{$video->order}}"
                                                   name="order"
                                                   type="number" value="{{$video->order}}">
                                        </div>
                                        <div class="form-group">
                                            <label for="upload_by" class="control-label upload_by" aria-required="true">
                                                {{__('nguonbaiviet')}}
                                            </label>
                                            <input class="form-control " placeholder="{{$video->post_source}}"
                                                   name="post_source"
                                                   type="text" value="{{$video->post_source}}">
                                        </div>
                                    </div>
                        ...
                        <div class="col-md-3 right-sidebar">
                            ...
                            <div class="widget meta-boxes">
                                ...
                                <div class="widget-body">
                                    <div class="ui-select-wrapper">
                                    {!! Form::select('status', $listStatus, $video['status'], ['class' => 'form-control ui-select ui-select', 'id'=>'status']) !!}
                                        <svg class="svg-next-icon svg-next-icon-size-16">
                                            <use xmlns:xlink="http://www.w3.org/1999/xlink"
                                                 xlink:href="#select-chevron"></use>
                                        </svg>
                            ...
                            <!-- <div class="widget meta-boxes">
                                <div class="widget-title">
                                    <h4><label for="format_type" class="control-label required"
                                        >{{__('dinhdang')}}</label></h4>
                                </div>
                                <div class="widget-body">
                                    <div class="mt-radio-list">
                                    <label>
                                        <input type="radio" value="1" name="format_type" {{$video["format_type"] == 1?'checked':''}}>
                                        News
                                    </label>
                                    <label>
                                        <input type="radio" value="2" name="format_type" {{$video["format_type"] == 2?'checked':''}}>
                                        Video
                                    </label>
                                    </div>
                                </div>
                                    ...
                                    <select id="tournament-id" class="tournament w-100" data-show-subtext="true" data-live-search="true"
                                        ...
                                        @if(is_array($getListTournamentOrderByTime) || is_object($getListTournamentOrderByTime))
                                            @foreach($getListTournamentOrderByTime as $item)
                                                <option class="tournament-item" data-id="{{$item->id}}" 
                                                        value="{{$item->id}}" 
                                                        @if($video["tournament_id"]==$item->id) 
                                                        selected=""
                                                        @endif
                                                        >{{$item->name}}
                                                </option>
                                                <!-- data-subtext="{{$item->link_livescore}}" -->
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                                ...
                                <div class="widget-title">
                                    <h4>
                                        <label for="category" class="control-label required"
                                               aria-required="true">{{__('chuyenmuc')}}</label>
                                    </h4>
                                </div>
                                <div class="widget-body">
